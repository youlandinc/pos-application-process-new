...
const RootStore = types.model(RootModel).actions((self) => {
  return {
    ...
    logout(isMemo = true) {
      // Multiple requests may be issued while logging in to expire, ensuring that only the first returned request is logOut operation, and the request is no longer repeated
      ...
      //     query: {
      //       redirectUrl: isMemo ? window.location.href : '',
      //     },
      //   },
      //   '/auth/sign_in',
      // );
