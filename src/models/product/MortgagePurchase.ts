...
export const MortgagePurchase = types
  ...
  .actions((self) => ({
    ...
    getApplicationProgressSnapshot(): ApplicationProcessSnapshot {
      const { state } = self;
    ...
    setClientState(clientAppProgress: ApplicationProcessSnapshot) {
      ...
      variables.forEach((variable) => {
        switch (variable.name as VariableName) {
          case VariableName.starting: {
            const value = variable.value as StartingData;
          ...
          case VariableName.propertyNew: {
            const value = variable.value as PropertyNewData;
