...
export const CreditScore = types
    ...
    return {
      ...
      debtTotalAmount: (role: 'self' | 'coBorrower' = 'self') => {
        ...
        const total = self.values[key].reduce(
          (cur, next) => cur + next.payment,
          0,
        );

        return self.values[key].reduce((total, debt) => {
          return debt.disabled ? total - debt.payment : total;
        }, total);
      },
      reducingMonthlyPayment: (role: 'self' | 'coBorrower' = 'self') => {
        return self.values[
          ...
          return debt.canPayoff ? sum + debt.payment : sum;
        }, 0);
      },
      get loanBalance() {
        return self.values.borrowerDebts.reduce((sum, debt) => {
          return debt.consolidate ? sum + debt.balance : sum;
        }, 0);
      },
  ...
  .actions((self) => ({
    ...
    injectCoBorrowerRelationshipData: (value: AboutOtherRelationData) => {
      const {
        isOnTheTitle,
        isCoBorrower,
        relationshipOpt,
    ...
    getCoBorrowerRelationPostData: (): Variable<AboutOtherRelationData> => {
